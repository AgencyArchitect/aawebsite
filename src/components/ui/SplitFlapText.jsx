...
const normalizePhrase = (phrase, width) => {
  ...
  const text = safe.slice(0, width);
  // Centreer de tekst horizontaal binnen de vaste boardbreedte (pad gelijk
  // over beide kanten), zodat korte woorden niet links uitlijnen maar midden staan.
  const totalPad = Math.max(0, width - text.length);
  const padBefore = Math.floor(totalPad / 2);
  const padAfter = totalPad - padBefore;
  return ' '.repeat(padBefore) + text + ' '.repeat(padAfter);
};
